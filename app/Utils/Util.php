...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        for ($i = 1; $i <= 7; $i++) {
            if (! empty($custom_labels['payments']['custom_pay_' . $i])) {
                $payment_types['custom_pay_' . $i] = $custom_labels['payments']['custom_pay_' . $i];
            }
        }
    ...
    {
        $date_format = session('business.date_format') ?: config('constants.default_date_format', 'd/m/Y');
        $mysql_format = 'Y-m-d';
        if ($time) {
        ...
        try {
            return ! empty($date_format) && ! empty($date) ? \Carbon::createFromFormat($date_format, $date)->format($mysql_format) : (! empty($date) ? \Carbon::parse($date)->format($mysql_format) : null);
        } catch (\Exception $e) {
